...
class PaymentTerm(ModelSQL, ModelView):
    'Payment Term'
    _name = 'ekd.document.payment_term'
    _description = __doc__
    name = fields.Char('Payment Term', size=None, required=True, translate=True)
    active = fields.Boolean('Active')
    description = fields.Text('Description', translate=True)
    lines = fields.One2Many('ekd.document.payment_term.line', 'payment',
            'Lines')
    ...
    def compute(self, amount, currency, payment_term, date=None):
        ...
        type_obj = self.pool.get('ekd.document.payment_term.line.type')
        delay_obj = self.pool.get(
                'ekd.document.payment_term.line.delay')
...
class PaymentTermLineType(ModelSQL, ModelView):
    ...
    _name = 'ekd.document.payment_term.line.type'
    _description = __doc__
...
class PaymentTermLineDelay(ModelSQL, ModelView):
    ...
    _name = 'ekd.document.payment_term.line.delay'
    _description = __doc__
    name = fields.Char('Name', size=None, translate=True, required=True)
    code = fields.Char('Code', size=None, required=True)
...
class PaymentTermLine(ModelSQL, ModelView):
    'Payment Term Line'
    _name = 'ekd.document.payment_term.line'
    _description = __doc__
    sequence = fields.Integer('Sequence',
            help='Use to order lines in ascending order')
    payment = fields.Many2One('ekd.document.payment_term', 'Payment Term',
    ...
    def get_delay(self):
        delay_obj = self.pool.get('ekd.document.payment_term.line.delay')
